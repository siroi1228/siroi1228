...
{/* 자기소개 섹션 끝 */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className={`group p-8 rounded-[2rem] border transition-all duration-500 flex flex-col h-full ${
            isDark 
              ? 'bg-slate-800/50 border-slate-700 hover:border-indigo-500 hover:shadow-[0_0_30px_-10px_rgba(99,102,241,0.5)]' 
              : 'bg-white border-gray-100 shadow-lg hover:shadow-2xl hover:border-blue-200'
          }`}>
            <div className="mb-6">
            ...
            <Link href="/goals" className="mt-auto">
              <button className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold hover:bg-blue-700 transition-colors shadow-lg">
                목표 상세보기
              </button>
            </Link>
          </div>
          ...
          <div className={`group p-8 rounded-[2rem] border transition-all duration-500 flex flex-col h-full ${
            isDark 
              ? 'bg-slate-800/50 border-slate-700 hover:border-emerald-500 hover:shadow-[0_0_30px_-10px_rgba(16,185,129,0.5)]' 
              : 'bg-white border-gray-100 shadow-lg hover:shadow-2xl hover:border-emerald-200'
          }`}>
            <div className="mb-6">
              <span className="text-4xl mb-4 block group-hover:scale-110 transition-transform">💻</span>
            ...
            <Link href="/projects" className="mt-auto">
              <button className="w-full bg-emerald-600 text-white py-3 rounded-xl font-bold hover:bg-emerald-700 transition-colors shadow-lg">
                프로젝트 보기
              </button>
            </Link>
          </div>
...
<div className={`group p-8 rounded-[2rem] border transition-all duration-500 flex flex-col h-full ${
  isDark 
    ? 'bg-slate-800/50 border-slate-700 hover:border-orange-500 hover:shadow-[0_0_30px_-10px_rgba(249,115,22,0.5)]' 
    : 'bg-white border-gray-100 shadow-lg hover:shadow-2xl hover:border-orange-200'
}`}>
  <div className="mb-6 text-center md:text-left">
    ...
    <p className={isDark ? 'text-slate-400' : 'text-gray-500'}>
      공부한 내용과 일상을 기록하는<br/>티스토리 블로그입니다.
    </p>
  </div>
  
  {/* 외부 링크이므로 Link 태그 대신 a 태그를 사용하거나 Link의 href에 전체 주소를 넣습니다. */}
  <a 
    href="https://note82441.tistory.com/" // 여기에 실제 블로그 주소를 넣으세요!
    target="_blank" 
    rel="noopener noreferrer"
    className="mt-auto"
  >
    <button className="w-full bg-orange-500 text-white py-4 rounded-2xl font-black hover:bg-orange-600 transition-colors shadow-lg shadow-orange-500/20">
      블로그 방문하기 →
    </button>
  </a>
</div>
          ...
          <div className={`group p-8 rounded-[2rem] border transition-all duration-500 flex flex-col h-full ${
            isDark 
              ? 'bg-slate-800/50 border-slate-700 hover:border-purple-500 hover:shadow-[0_0_30px_-10px_rgba(168,85,247,0.5)]' 
              : 'bg-white border-gray-100 shadow-lg hover:shadow-2xl hover:border-purple-200'
          }`}>
            <div className="mb-6">
              <span className="text-4xl mb-4 block group-hover:scale-110 transition-transform">📚</span>
            ...
            <Link href="/study" className="mt-auto">
              <button className="w-full bg-purple-600 text-white py-3 rounded-xl font-bold hover:bg-purple-700 transition-colors shadow-lg">
                학습기록 보기
              </button>
            </Link>
          </div>
          {/* 카드 5: 경력 및 이력 (메인 페이지 추가용) */}
<div className={`group p-8 rounded-[2rem] border transition-all duration-500 flex flex-col h-full ${
  isDark 
    ? 'bg-slate-800/50 border-slate-700 hover:border-pink-500 hover:shadow-[0_0_30px_-10px_rgba(236,72,153,0.5)]' 
    : 'bg-white border-gray-100 shadow-lg hover:shadow-2xl hover:border-pink-200'
}`}>
  <div className="mb-6">
    ...
    <p className={isDark ? 'text-slate-400' : 'text-gray-500'}>
      지금까지 걸어온 길과 <br/>주요 활동 기록입니다.
    </p>
  </div>
  <Link href="/career" className="mt-auto">
    <button className="w-full bg-pink-500 text-white py-4 rounded-2xl font-black hover:bg-pink-600 transition-all shadow-lg shadow-pink-500/20 active:scale-95">
      이력 확인하기 →
    </button>
  </Link>
</div>
